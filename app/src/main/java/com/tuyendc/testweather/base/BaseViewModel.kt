package com.tuyendc.testweather.base
...
open class BaseViewModel @Inject constructor (application: Application) : AndroidViewModel(application) {
    ...
    var loadingEvent: SingleLiveEvent<Boolean> = SingleLiveEvent()
    ...
    open fun launchDataLoadWithoutProgress(block: suspend () -> Unit): Job {
        return viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                Log.d("TT.QQ error",""+ error.message)
    ...
    open fun onRefresh(
        coroutineDispatcher: CoroutineDispatcher = Dispatchers.IO, block: (suspend () -> Unit)?
    ): Job {
